...
export class ReportsService {
  ...
  private cacheKey(type: string, params: Record<string, unknown>): string {
  ...
  async projectStats(projectId: string) {
    const key = this.cacheKey('projectStats', { projectId });
    return this.cached(key, async () => {
      const entries = await this.prisma.timeEntry.findMany({
      ...
      for (const entry of entries) {
        ...
        const current = byWorkType.get(wtId) ?? {
      ...
      return {
        data: {
          ...
          uninvoicedAmount:
            Math.round((totalAmount - invoicedAmount) * 100) / 100,
    ...
    return this.cached(key, async () => {
      ...
      const data = [...byUser.values()].map((u) => ({
        ...
        utilizationPercent:
          u.totalHours === 0
            ? 0
            : Math.round((u.billableHours / u.totalHours) * 10000) / 100,
      }));
