...
export class InvoicesService {
  ...
  async update(id: string, dto: UpdateInvoiceDto) {
    ...
    await this.assertRelations(
      ...
      dto.bankAccountId ?? existing.bankAccountId,
    );
    const invoice = await this.prisma.$transaction(async (tx) => {
      const data: Prisma.InvoiceUncheckedUpdateInput = {
        ...
        ...(dto.dueDate === undefined
          ? {}
          : { dueDate: new Date(dto.dueDate) }),
        ...(dto.taxPointDate === undefined
          ? {}
          : { taxPointDate: toNullableDate(dto.taxPointDate) }),
    ...
    this.webhooksService.fire('invoice.paid', {
  ...
  async send(id: string, user: AuthenticatedUser) {
    ...
    this.webhooksService.fire('invoice.sent', {
      invoiceId: invoice.id,
      invoiceNumber: invoice.invoiceNumber,
      clientId: invoice.clientId,
    });
  ...
  private async queueInvoiceEmail(invoice: {
    ...
    const total =
      typeof invoice.total === 'object' && invoice.total?.toNumber
    ...
    await this.emailService.queueInvoiceEmail(
      ...
      recipientEmail,
      {
        ...
        total: new Intl.NumberFormat('cs-CZ', {
