...
export function UnbilledReport({ onExportRef }: Props) {
  ...
  const fetchReport = async () => {
    try {
      setIsLoading(true);
      const res = await api.get<{ data: UnbilledProject[] }>('/reports/billing');
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-sm font-medium text-zinc-400">Total Unbilled Hours</div>
            <div className="text-2xl font-bold text-zinc-100">{totalHours.toFixed(2)}h</div>
          </div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-sm font-medium text-zinc-400">Total Unbilled Amount</div>
            <div className="text-2xl font-bold text-zinc-100">${totalAmount.toFixed(2)}</div>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-zinc-400">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-zinc-500">Loading billing data...</td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-zinc-500">No unbilled time found</td>
                </tr>
              ) : (
                data.map((project) => (
                  <React.Fragment key={project.projectId}>
                    ...
                    {expanded[project.projectId] && (
                      <tr>
                        <td colSpan={4} className="p-0 border-b border-zinc-800">
                            ...
                            <table className="w-full text-xs text-zinc-500">
                              ...
                              <tbody>
                                {project.entries.map((entry) => (
                                  <tr key={entry.id} className="border-b border-zinc-900/50 hover:bg-zinc-900/80">
                                    <td className="py-2">{new Date(entry.startedAt).toLocaleDateString()}</td>
                                    <td className="py-2">{entry.user?.name || 'Unknown'}</td>
                                    <td className="py-2 max-w-xs truncate" title={entry.description || ''}>
                                      {entry.description || '—'}
                                    </td>
                                    <td className="py-2 text-right">
                                      {entry.durationMinutes ? (entry.durationMinutes / 60).toFixed(2) : '0.00'}
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
            ...
            {!isLoading && data.length > 0 && (
              <tfoot className="bg-zinc-900 font-bold border-t-2 border-zinc-700">
                <tr>
                  <td colSpan={2} className="px-6 py-4 text-zinc-100">Total Unbilled</td>
                  <td className="px-6 py-4 text-right text-zinc-100">{totalHours.toFixed(2)}h</td>
                  <td className="px-6 py-4 text-right text-emerald-400">${totalAmount.toFixed(2)}</td>
                </tr>
              </tfoot>
            )}
          </table>
