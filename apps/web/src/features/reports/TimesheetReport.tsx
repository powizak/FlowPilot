...
export function TimesheetReport({ dateFrom, dateTo, onExportRef }: Props) {
  ...
  useEffect(() => {
    api.get<{ data: User[] }>('/users').then(res => setUsers(res.data.data)).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    if (onExportRef) {
      onExportRef(() => {
        downloadCsv('/reports/timesheet', { dateFrom, dateTo, userId: userId || undefined, groupBy }, 'timesheet_report');
      });
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row gap-4 bg-zinc-900 border border-zinc-800 p-4 rounded-xl">
        ...
        <div className="flex-1">
          <label className="block text-sm font-medium text-zinc-400 mb-1">Group By</label>
          <select 
            value={groupBy} 
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="period" stroke="#71717a" fontSize={12} tickLine={false} axisLine={false} />
              ...
              <Bar dataKey="billableHours" name="Billable" stackId="a" fill="#3b82f6" radius={[0, 0, 4, 4]} />
              <Bar dataKey="nonBillable" name="Non-Billable" stackId="a" fill="#6366f1" radius={[4, 4, 0, 0]} opacity={0.5} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
            ...
            <tbody>
              {data.map((row, idx) => (
                <tr key={idx} className="border-b border-zinc-800/50 hover:bg-zinc-800/20">
                  <td className="px-6 py-3 font-medium text-zinc-300">{row.period}</td>
