      ...
      <div className="w-16 border-r border-border bg-sidebar shrink-0 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          {hours.map((h) => (
            <div
              key={h}
              className="h-16 border-b border-border text-xs text-text-secondary text-right pr-2 pt-1.5 font-medium select-none"
            >
        ...
        {dayEntries.map((entry) => {
          ...
          return (
            <div
              key={entry.id}
              className={cn(
                'absolute left-2 right-4 rounded-md border border-background z-10 p-2 text-xs overflow-hidden shadow',
                colorClass,
              )}
