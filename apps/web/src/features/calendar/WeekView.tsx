    ...
    <div className="flex flex-1 h-full min-h-0 border border-border rounded-lg overflow-hidden bg-background">
        ...
        <div className="flex-1 overflow-hidden relative">
          <div className="absolute inset-0 pointer-events-none">
            {hours.map((h) => (
              <div
                key={h}
                className="h-12 border-b border-border text-[10px] text-text-secondary text-right pr-2 pt-1 font-medium select-none"
              >
      ...
      <div className="flex-1 flex overflow-hidden">
        {week.map((day) => {
          ...
          const dayEntries = timeEntries.filter((t) =>
            isSameDay(new Date(t.startTime), day.date),
          );
          const dayKey = day.isoString || day.date.toISOString();

          return (
            <button
              key={dayKey}
              type="button"
              className="flex-1 border-r border-border last:border-r-0 flex flex-col min-w-[100px] relative cursor-pointer text-left"
                ...
                {dayEntries.map((entry) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className={cn(
                        'absolute left-1 right-1 rounded border border-background z-10 p-1 text-[10px] overflow-hidden shadow-sm',
                        colorClass,
                      )}
                    ...
                    >
                      <div className="font-semibold text-white/90 truncate leading-tight">
                        {entry.description || 'No description'}
                      </div>
                      {height >= 24 && (
                        <div className="text-white/70 truncate text-[9px] mt-0.5">
