...
export const GanttTimeline: React.FC<Props> = ({ tasks, scale }) => {
  const { zoom, pixelsPerDay, startDate, totalWidth, totalDays } = scale;
  const rowHeight = 40;
  ...
  tasks.forEach((t, i) => coordsMap.set(t.id, getTaskCoords(t, i)));
  ...
  return (
    <div className="relative" style={{ width: totalWidth, height: '100%' }}>
      <svg width="100%" height={tasks.length * rowHeight} className="absolute inset-0 block pointer-events-none z-10">
        <defs>
        ...
        {tasks.map((t, i) => {
          ...
          const isMilestone = t.startDate && t.dueDate && sDate.getTime() === dDate.getTime();
          ...
          if (isMilestone) {
            ...
            return <path key={`bar-${t.id}`} d={diamondPath} fill={fill} className="pointer-events-auto" />;
          }

          return (
            <g key={`bar-${t.id}`} className="pointer-events-auto cursor-pointer hover:opacity-80">
              <rect x={x} y={y} width={w} height={h} fill={fill} rx="4" opacity={0.3} />
              <rect x={x} y={y} width={progressW} height={h} fill={fill} rx="4" />
            </g>
          );
        })}
      </svg>
