...
export function InvoiceForm() {
  ...
  useEffect(() => {
    ...
    api.get('/bank-accounts').then(({ data }) => setBankAccounts(data.data || []));
  ...
  const { subtotal, vatAmount, total } = useMemo(() => {
    const sub = formData.lineItems?.reduce((sum, item) => sum + (item.total || 0), 0) || 0;
    const vat = formData.lineItems?.reduce((sum, item) => sum + ((item.total || 0) * (item.vatPercent || 0) / 100), 0) || 0;
  ...
  const handleTimeEntriesAdd = async (projectId: string, dateFrom: string, dateTo: string) => {
    try {
      const { data } = await api.post('/invoices/from-entries', {
        projectId, dateFrom, dateTo, clientId: formData.clientId, dueDate: formData.dueDate
      });
      navigate(`/invoices/${data.id}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-[#1a1a1a] border border-[#2d2d2d] rounded-lg p-6">
        ...
        <LineItemsTable
          ...
          onChange={(items) => setFormData({ ...formData, lineItems: items as LineItem[] })}
